...
| high_speed_comparator | 1.222× | 1.132× | 48 s | 4/10 — 계층, 아래 "남은 것" (WebGPU 설정당 8 개면 1.111× / 1.109×) |
...
남은 유일한 자리다. WebGPU 로 설정당 시작점 8 개(2,304 개)를 주면 1.111x / 1.109x
로 온다 — 표본이 늘면 좋아지는 방향이다. 실제 GPU 에서 32 개로 재는 것이 다음이다.)
