...
edit.mjs          편집기 — 보기/편집 토글로 켜는 배치·배선 편집. index.html 이 필요할 때 동적으로 받는다 (못 받아도 보기·배치·배선은 된다)
...
4. **편집 (선택)** — 캔버스 제목 줄의 **보기 / 편집** 토글. Placement·Routing 보기에서 손으로 고치고, 고친 위상 안에서
   다시 최적화한다 (아래 "편집" 절). 편집을 켜면 편집 카드가 캔버스 바로 옆 결과 카드 자리에 오고 나머지 카드는 접힌다.
   편집하지 않으면 전부 위와 같다 — 배치기·배선기가 다 고르고, 결과와 파일도 같다.

캔버스 옆과 아래의 카드(결과, 설계, Routing · GDS · DRC, 소자 variant, Primitives 블록 표, 하위 모듈)는 접혀 있는 것이 기본이다 —
제목을 클릭하면 펴진다. Placement·Routing 보기의 그림은 캔버스 좌우를 꽉 채우고 높이는 비율대로 따라간다 (창 높이의 1.4 배까지 —
그보다 세로로 긴 설계는 다 보이게 맞춘다).

그림은 흐름 순서대로 넷이다 — `Schematic` / `Primitives` / `Placement` / `Routing`. 휠로 확대, 끌어서 이동,
더블클릭으로 초기화. Routing 보기에서는 금속·비아·소자층·웰을 묶음으로 껐다 켤 수 있다.

- **Schematic** — 넷리스트(.sp)를 그대로 그린 것. 트랜지스터 하나하나가 기호로 놓이고, 서브서킷 계층은
...
### 편집 — 위상은 사용자가 정하고, 나머지는 최적화기가

편집 모드의 원칙은 셋이다. **편집은 제약을 따라 움직인다** — 거울 쌍의 한쪽을 끌면 짝이 거울로 따라오고, 축 위 블록은
축을 따라서만 미끄러진다. 배치기의 영공간 `z = z0 + N theta` 위에서 끌기 때문에 끄는 동안에도 대칭 잔차가 0 이다.
**최적화는 사용자가 정한 위상 안에서만 한다** — 배치의 위상은 쌍마다의 왼/오/아래/위, 배선의 위상은 넷의 조각·비아·핀 접속이고,
최적화기는 좌표만 바꾼다. **편집이 없으면 아무것도 달라지지 않는다.** 계획과 실측은 [symplace/PLAN-edit.md](symplace/PLAN-edit.md).

**Placement 편집.** 블록을 클릭해 고르고 (Shift 로 여럿) 끌면 제약을 따라 움직인다. 축 핸들을 끌면 축이 움직이고, Alt 를
누른 채 블록을 끌면 그 대칭 그룹이 통째로 간다. 끄는 동안 겹친 쌍은 빨갛고, 원래 자리는 유령 상자로, 고른 블록의 넷은
플라이라인으로 보인다. 놓은 뒤 **정리** (Enter) 를 누르면 편집 좌표에서 읽은 쌍 관계를 그대로 둔 채 겹침을 없애고 격자에
앉히며 압축한다 — 배치기의 legalize 그 자체라 수십 ms 다. 압축 슬라이더가 0 이면 손댄 것만 고치고, 배치기 기본 0.5 면
배치기가 냈을 배치로 돌아간다. `Order` 를 어긴 편집(순서 제약이 있는 블록의 자리 바꾸기)은 정리가 어느 쌍인지 알리며 거절한다.

| 키 | |
|---|---|
| 화살표 | 한 격자 (80 / 84) 씩, Shift 면 열 격자 |
| F / V | x / y 반전 (거울 그룹 단위) |
| L | 잠금 — 끌 때 그 자리에 둔다 |
| S | 고른 두 블록의 자리 바꾸기 |
| Enter | 정리 |
...
편집한 배치는 그 배치의 배선 결과를 지운다 (Routing 을 다시 실행한다). 내려받는 배치 JSON 이름에 `-edited` 가 붙고
안에 `edited` (편집 수) 와 `fixedVariants` 가 적힌다.

**Routing 편집.** 도형을 클릭하면 그 넷이 골라지고 나머지는 흐려진다. 그 넷의 조각(같은 트랙의 토막을 이은 것)을 클릭하면
옮길 수 있는 범위가 카드에 뜬다 — 핀 위 조각은 못 옮기고, 이웃 조각의 최소 길이와 끝단 간격, 같은 층의 다른 도형과의
간격, 비아 간격, 모듈 경계가 범위를 정한다. 끌거나 화살표로 옆 트랙에 옮기면 (층의 수직 방향으로만, 트랙에 스냅)
배선기 없이 도형 합성·DRC/LVS·GDS 를 다시 해 (10~20 ms) 카드가 배선기 결과 대비 오류 수를 보이고, 그 넷은 **고정**된다.

| 키 | |
|---|---|
| 화살표 | 한 트랙 |
| F | 이 넷 고정 / 해제 — 고정한 넷은 다시 배선할 때 그대로 둔다 (배선기 입력에 장애물로 심는다) |
| R | 이 넷만 다시 배선 — 나머지 넷은 지금 경로 그대로 고정 |
| C / Shift+C | 정돈 — 이 넷 / 전부. 위상(조각·비아·핀 접속)은 그대로, 조각을 범위 안에서 미끄러뜨려 길이를 줄인다. 오류가 늘면 되돌린다 |
| Z / Y | 되돌리기 / 다시 실행 |
| Esc | 조각, 다음에 넷 고르기 해제 |
