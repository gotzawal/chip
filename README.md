...
## 배치 품질 (ALIGN 대비, 시작점 96, node 단일 스레드)

| 예제 | 면적 | HPWL | 시간 | 변이 |
|---|---|---|---|---|
| telescopic_ota | **1.000×** | 0.987× | 9 s | ALIGN 과 같은 것을 골랐다 (5/5) |
| current_mirror_ota | **1.000×** | **1.000×** | 15 s | 같다 (5/5) |
| five_transistor_ota | **0.862×** | 1.404× | 31 s | 셋 중 하나만 같다 |
| cascode_current_mirror_ota | 1.025× | 1.255× | 66 s | 모양은 11/11 같다 |
| high_speed_comparator | **0.965×** | 1.084× | 177 s | 모양은 9/10 같다 |
...
1. **점수가 다르다.** 우리 점수는 `면적/refArea + hpwlWeight x HPWL/refHpwl
   + 3 x 겹침` 이고 기본 무게에서 면적이 조금 앞선다. five_transistor_ota 에서
   ALIGN 과 다른 변이를 고르는 것이 이것 때문이다 — 면적을 14% 얻고 배선을
   40% 내주는 맞바꿈이다. 배선을 더 보려면 `hpwlWeight` 를 올리면 된다.
   (`placeDesign` 의 인자다.)
