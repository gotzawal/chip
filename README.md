...
첫 화면(배치 그림·지표·변이 표)은 **워커 없이** 뜬다. 모듈 워커를 못 쓰는
...
페이지를 열면 배치가 바로 두 개 보인다. ALIGN 기준선은 앞단 출력에서 그 자리에서
뽑고, 우리 쪽은 미리 계산해둔 결과(`downloads/*.place.json`)를 그린다. 배치 실행을
누르면 그 자리에 이번에 푼 것이 들어온다.
