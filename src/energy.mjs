 ...
 */
export function wirelength(px, py, pinNet, nNet, gamma) {
  const P = px.length;
  let W = 0;
  const gx = new Float64Array(P);
  const gy = new Float64Array(P);

  for (const [pos, grad] of [[px, gx], [py, gy]]) {
    ...
    for (let p = 0; p < P; p++) {
      const e = pinNet[p], x = pos[p];
      const up = Math.exp((x - hi[e]) / gamma);
      const vp = Math.exp((lo[e] - x) / gamma);
      u[p] = up; v[p] = vp;
      Su[e] += up; Sv[e] += vp;
      Tu[e] += x * up; Tv[e] += x * vp;
    }
    ...
    for (let p = 0; p < P; p++) {
      const e = pinNet[p], x = pos[p];
      const dA = (u[p] / Su[e]) * (1 + (x - A[e]) / gamma);
      const dB = (v[p] / Sv[e]) * (1 - (x - B[e]) / gamma);
      grad[p] += dA - dB;
    }
  }
  return { W, gx, gy };
...
export class Density {
  ...
  eval(cx, cy, w, h) {
    ...
    // rho[m][n] = sum_i ox[i][m] * oy[i][n] / cell
    const rho = new Mat(Mx, My);
    for (let i = 0; i < N; i++) {
      ...
      for (let m = 0; m < Mx; m++) {
        ...
        if (a === 0) continue;
        const rm = m * My;
        for (let n = 0; n < My; n++) rho.data[rm + n] += (a * oy.data[by + n]) / cell;
      }
    }
    let mean = 0;
    for (let i = 0; i < rho.data.length; i++) mean += rho.data[i];
    mean /= rho.data.length;
    for (let i = 0; i < rho.data.length; i++) rho.data[i] -= mean;   // 균일 중화 배경

    // R = Cx rho Cy^T,  psi = Cx^T (R * invDen) Cy
    const R = matmul(matmul(this.Cx, rho), this.Cy.transpose());
    for (let i = 0; i < R.data.length; i++) R.data[i] *= this.invDen.data[i];
    const psi = matmul(matmul(this.Cx.transpose(), R), this.Cy);

    let D = 0;
    for (let i = 0; i < rho.data.length; i++) D += rho.data[i] * psi.data[i];
    D *= 0.5 * cell;

    // D = (cell/2) rho^T G rho -> dD/drho = cell * psi 이고
    ...
    for (let i = 0; i < N; i++) {
      ...
      let sx = 0;
      for (let m = 0; m < Mx; m++) {
        const dm = dox.data[bx + m];
        if (dm === 0) continue;
        const pm = m * My;
        let t = 0;
        for (let n = 0; n < My; n++) t += psi.data[pm + n] * oy.data[by + n];
        sx += dm * t;
      }
      gcx[i] = sx;
      let sy = 0;
      for (let n = 0; n < My; n++) {
        const dn = doy.data[by + n];
        if (dn === 0) continue;
        let t = 0;
        for (let m = 0; m < Mx; m++) t += psi.data[m * My + n] * ox.data[bx + m];
        sy += dn * t;
      }
      gcy[i] = sy;
    }
