...
**방향 바뀜 (2026-09-23).** 새 배선기는 ALIGN 과 결과가 다른 독립 배선기라 쓸모가 없다고 봤다.
배선기는 ALIGN C++ 배선 알고리즘을 그대로 따라 **같은 배치에서 ALIGN 과 같은 도형**을 내도록
Rust 로 옮긴다 — [PLAN-route-align.md](PLAN-route-align.md). 파이썬을 뺀 앞뒤(검사기·도형 합성·GDS·
리프 도형·배선 워커)는 그대로 쓴다. 지금 배선기가 ALIGN 과 다른 점과 한계는 5 절.
...
> 2026-09-23: 이 판단은 접었다. 결과가 ALIGN 과 같아야 해서 C++ 알고리즘을 그대로 옮긴다
> (5 절, [PLAN-route-align.md](PLAN-route-align.md)). 아래는 그때의 근거로 남겨 둔다.
...
- **끝남** (`src/route/problem.mjs`, `test/problem.mjs`). 계층을 원래 topology 로 펼친다
  (포트 -> 부모 넷, 전역 전원 그대로, 안쪽 넷은 `<경로>/<넷>` — ALIGN 이 계층 배선 끝에 최상위에서
  쓰는 이름과 같다). 펼친 리프 도형을 검사기로 합치면 **배선 전의 OPEN 이 곧 이을 일**이다:
  넷마다 연결 덩이(component)들. 배선기에는 배선층의 합친 도형(넷·덩이·핀 표시)과 층·비아 규칙,
  대칭 넷 쌍과 축(같은 모듈의 SymmetricBlocks 에서)을 넘긴다. 5 예제 모두 배선 전 SHORT·DRC 0.
  ALIGN 배치(`data/<예제>.json` 의 place)를 같은 모양으로 바꾸는 `placementFromAlign` 이 있어
  캐시 없이도 시험된다. 알게 된 것: 한 소자 안에서 대칭인 넷(차동쌍의 DA/DB)은 리프 안의 도형이
  거울상이 아니다 (M2 레일의 y 가 다르다) — 대칭은 "거울 경로를 선호" 로 풀어야 한다.
...
- **대칭 넷** (`sym.rs`): 쌍을 먼저, 앞선 넷을 잇고 따르는 넷은 그 거울상을 먼저 해 본다 (노드가
  모두 쓸 수 있고, 다른 넷과 안 겹치고, 덩이를 다 잇고, 토막 규칙을 지키면 그대로). 안 되면 거울
  노드에 값을 깎아 주는 A*. 앞선 넷은 축에 붙는 노드(거울상이 자기와 겹치는 곳)에 값을 더 문다.
  거울 그대로가 되는 것은 앞선 넷의 핀이 한쪽에만 있고 리프 도형도 거울상일 때뿐이다 — 축을
  가로지르는 넷은 가로 트랙에서 제 거울상과 같은 트랙에 겹치고, 차동쌍 리프(DA/DB)는 핀 레일의
  y 가 달라서다. 그래서 잣대는 두 넷의 배선 길이 차다: 우리 최대 30 % (1 µm 안 되는 넷), ALIGN 도
  같은 배치에서 27~58 % 가 있다. comparator 는 ALIGN 배치에서 한 쌍이 거울 그대로다.
- **전원 넷**: 신호처럼 잇되 맨 나중에. ALIGN 처럼 M5/M6 격자를 치지 않는다 (검사기는 0).
- **계층**: 펼쳐서 한 번에 (problem.mjs). 계층 설계 둘도 DRC/LVS 0.
...
## 5. 지금 배선기 — ALIGN 과 다른 점과 한계 (2026-09-23)

**방향이 바뀌었다.** 결과가 ALIGN 과 다른 독립 배선기는 쓸모가 없다 — 같은 배치에서 ALIGN 과
같은 배선이 나와야 한다. 그래서 배선기를 ALIGN 의 C++ 배선 알고리즘을 그대로 따르도록 다시 짠다.
계획은 [PLAN-route-align.md](PLAN-route-align.md). 이 절은 3·4 단계에서 만든 배선기가 무엇이고
어디까지인지 적어 둔다. 이식이 합격하면 이 배선기는 걷어낸다.

### 5.1 옮긴 것과 새로 짠 것

ALIGN 에서 **그대로 옮긴 것은 파이썬 쪽 심판과 출력**이다. 모두 파이썬과 결과를 대조했다.

| ALIGN 원본 (파이썬) | 옮긴 곳 | 대조 |
|---|---|---|
| `cell_fabric` 검사기 (remove_duplicates · drc · postprocess) | `src/route/check.mjs` | 116 사례에서 오류 목록과 정리된 도형이 글자 그대로 |
| `gen_viewer_json` 의 도형 합성과 격자 검사 | `src/route/compose.mjs` | 10 모듈, 격자 문구 150 개 |
| `gen_gds_json.translate` + `json2gds` | `src/route/gds.mjs` | 최상위 `.python.gds` 와 바이트까지 |
| `manipulate_hierarchy` 의 전원 포트 걷어내기 | `src/route/hier.mjs` | 3_pnr/inputs 의 verilog.json 과 |

원본과 다르게 둔 것은 원본이 죽는 두 입력(비아 밑에 금속이 없음)뿐이다 — JS 는 DRC 오류 줄로 적는다.

**배선기는 옮기지 않고 새로 짰다** (4.1 의 판단). 알고리즘부터 다르다.

| | ALIGN (`PlaceRouteHierFlow/router`, C++ 약 2.3 만 줄) | 지금 배선기 (`symplace/router`, Rust 1,447 줄) |
|---|---|---|
| 배선층 | 신호 M1~M5, 전원 M1~M6 (층마다 피치가 다르다) | M2~M4 (넓혀도 M1~M4) |
| 전역 배선 | 칸(gcell) 단위, 넷마다 후보 트리 5 개, lp_solve 로 0-1 ILP 를 풀어 하나씩 고른다 | 없다 |
| 상세 배선 | 전역 배선이 고른 칸 안에서 핀을 가까운 순으로 하나씩 A*, 재배선·협상 없음 | 한 격자에서 넷마다 A*, 겹치면 협상(PathFinder) |
| 규칙 | 다른 넷 도형을 끝단 간격만큼 불려 격자 점을 끄고, 비아 둘러싸기·간격으로 비아 자리를 끈다. 최소 길이는 A* 중 검사 + 끝에 늘리기 | 검사기 규칙에 맞춘 트랙별 구간 규칙, 끝에 최소 길이·끝단 간격 정리 |
| 대칭 넷 | 전역 배선에서는 사실상 꺼져 있다(축 좌표가 늘 -1). 상세 배선이 짝 넷 경로의 거울상 쪽으로 약하게 당긴다 | 앞선 넷의 거울상을 먼저 해 보고, 안 되면 거울 쪽을 권하는 A* |
| 전원 | 최상위에서 M5/M6 전원 격자를 치고 전원 핀을 격자로 잇는다 | 신호처럼 가는 선으로 맨 나중에 |
| 계층 | 하위 모듈부터 배선해 결과를 상위의 블록으로 넣는다 (bottom-up) | 펼쳐서 한 번에 |

같은 시험지(검사기)를 통과하는 다른 답안이라, 같은 배치에서도 도형이 ALIGN 과 다르다.
ALIGN 의 옛 C++ 경로(Pyodide + 축소 PnR 휠 + `pyroute.py`)는 node 하네스에만 기준으로 남아 있다.

### 5.2 한계

1. **"DRC/LVS 0" 은 ALIGN 검사기 기준이다** (사인오프 DRC 가 아니다).
   - 보는 것: 같은 층·같은 트랙(중심선) 위 도형끼리의 합선·끝단 간격·최소 길이, x 나 y 가 같은
     비아끼리의 간격, 금속 방향의 비아 둘러싸기, 색 격자.
   - 안 보는 것: 이웃 트랙 사이 간격, 대각선 비아 간격, 최소 면적, 밀도, 안테나, 넓은 금속 규칙.
   - 사각지대: 중심선이 다른 두 도형은 겹쳐도 합선으로 잡지 않는다. 지금은 리프와 배선이 전부
     트랙 위 표준 폭이라 걸릴 일이 없다 (5 예제 전수 확인). 다른 PDK·리프 생성기에서는 놓칠 수 있다.
   - LVS 는 넷 이름표로 연결만 본다 (합선, 끊김, 한 단자가 두 넷에). 소자 추출과 넷리스트 대조는 없다.
   - 페이지 숫자가 0 이 아닐 때가 있다: current_mirror 4 건, cascode 는 배치에 따라 1~4 건. 리프 셀 안
     소자층(Rvt 등)의 DIFFERENT WIDTH 이고 ALIGN 결과에도 똑같이 있다.
2. **이 PDK 의 격자에 묶여 있다.**
   - 배선층이 한 격자여야 한다 (M1/M3 는 x 80, M2/M4 는 y 84). M5/M6(144)은 못 쓴다 — 범위에 넣으면
     "트랙 간격이 달라 한 격자에 못 싣는다" 로 멈춘다. 기본 M2~M4, 넓혀도 M1~M4.
   - 리프 금속이 전부 트랙 위·표준 폭이라는 전제에 기댄다. 아닌 도형은 닿는 트랙을 통째로 막는다
     (같은 넷도 못 붙는다).
   - "비아를 격자 노드에만 두면 간격은 늘 맞는다" 는 이 PDK 숫자에 딱 맞춘 논증이다 (V1 세로
     84 − 32 = 52 = SpaceY, 경계값).
   - PDK 표는 `src/route/pdk.mjs` 에 박혀 있다. 다른 PDK 는 격자 모델부터 다시 봐야 한다.
3. **전원이 약하다.** VDD/VSS 를 신호와 같은 가는 선(M2 32, M3·M4 40)으로 맨 나중에 잇는다. 전원 격자,
   굵은 선, IR drop/EM 고려가 없다. ALIGN 은 같은 배치에서 금속의 73~95 % 를 M5/M6 전원 격자에
   쓴다 — 비교표에서 우리 금속이 적은 것은 대부분 이 차이다.
4. **대칭 넷은 대부분 거울 그대로가 아니다.**
   - 거울 그대로인 쌍: comparator 의 ALIGN 배치 1/2, 브라우저에서 새로 푼 배치 1/2, 캐시의 우리
     배치 0/2. 나머지 예제는 0.
   - 기하적으로 안 되는 두 경우: 축을 가로지르는 넷(가로 트랙에서 제 거울상과 같은 트랙에 겹친다),
     차동쌍 셀(DA/DB 는 셀 안 핀 레일의 y 가 달라 핀부터 거울상이 아니다).
   - 그때는 거울 쪽을 권하는 A* 로 비슷하게만 친다. 쌍의 배선 길이 차 최대: telescopic 30 %
     (VOUTN/VOUTP, 0.4 대 0.6 µm), cascode 20 %, comparator 8 %, current mirror 1 %. ALIGN 도 같은
     배치에서 27 %, 58 % 가 있다 (핀 접점 도형이 섞인 숫자라 엄밀한 비교는 아니다).
   - 모양(과 길이)만 맞춘다. 비아 수, 이웃 넷과의 결합, 축에 걸친 한 넷(VCOM, CLK, 꼬리 전류)의
     자기 대칭은 보지 않는다.
5. **빽빽하면 못 잇는다.**
   - 빽빽한 합성 배치 1,400 개에서 성공 92.9 %. 실패한 넷의 절반(30)은 핀이 막혀 혼자서도 못 잇고,
     나머지(34)는 혼자는 되는데 협상이 못 풀었다. 반복을 60 -> 200 으로 늘려도, 매번 모든 넷을
     다시 이어도 같았다.
   - 보수적으로 둔 규칙: 같은 트랙에서 다른 넷과 노드 하나 띄우기, 두 노드 이상 토막에서만 비아,
     한 층 안에서 방향을 못 바꿈, 격자 밖 금지, M1 기본 안 씀, 배치 영역 밖 4 트랙까지.
   - 실패하면 그 넷을 통째로 걷어낸다 (합선 대신 OPEN, 상태 줄에 넷 이름).
   - 예제 다섯은 모두 첫 반복에 풀렸다 — 협상 경로는 합성 시험에서만 돌았다.
   - 배선이 막혀도 배치로 되돌아가 고치지 않는다.
6. **제약 대부분을 읽지 않고 조용히 무시한다.**
   - 읽는 것: SymmetricNets (축은 같은 모듈의 SymmetricBlocks 에서), PowerPorts / GroundPorts.
   - 무시하는 것: DoNotRoute (배선해 버리고, 검사에서 열림도 허용하지 않는다), Route (층 범위,
     차폐·매칭), NetConst (차폐 넷, 우선순위), MultiConnection (병렬 선), PortLocation, GuardRing.
   - 5 예제에는 이런 제약이 없어 시험에 걸리지 않았다.
7. **계층을 펼쳐서 친다.**
   - 하위 모듈을 한 번 배선해 복사하지 않고 인스턴스마다 따로 친다. comparator 의 XINV_N 과
     XINV_P 는 같은 하위 모듈인데 로컬 좌표로 배선 12 개 중 11 개만 같았다.
   - GDS 도 셀 계층 없이 평평한 한 구조다.
   - 하위 모듈 안의 대칭 제약은 그런 예제가 없어 확인하지 못했다.
8. **입력과 시험의 빈틈.**
   - 배선하려면 리프 셀 전체 도형이 필요하다. 예전에 "예제로 저장"한 파일이나 ALIGN 앞단 출력
     JSON 만으로는 안 된다 (페이지가 그렇게 알린다).
   - 커패시터·저항·가드링 셀은 예제에 없어 확인하지 못했다. 예전 경로는 커패시터를 거절했는데
     새 경로는 막지 않는다.
   - `.sp` 올리기는 브라우저로 못 돌렸다 (이 환경에서 CDN 이 막혔다 — node 에서 같은 앞단만 확인).
   - 큰 설계는 합성 배치로만 확인했다 (1,400 넷, 300 x 300 트랙, 0.7 s, 오류 0).
   - `router.wasm` 은 커밋된 바이너리다. Rust 를 고치면 `symplace/router/build.sh` 로 다시 빌드한다
     (같은 툴체인이면 바이트까지 같다).
...
| 한계 | 이식 뒤 |
|---|---|
| 1 심판 | 그대로다 — ALIGN 이 자기 결과를 재는 잣대다 |
| 2 격자 | ALIGN 배선기의 격자로 (신호 M1~M5, 전원 M1~M6, 층마다 다른 피치). 다른 PDK 는 ALIGN 이 되는 만큼 된다 |
| 3 전원 | ALIGN 의 M5/M6 전원 격자와 전원 배선 그대로 |
| 4 대칭 | ALIGN 과 같다. ALIGN 도 거울 그대로를 보장하지 않는다 — 전역 배선의 대칭은 꺼져 있고, 상세 배선이 약하게 당길 뿐이다 |
| 5 빽빽함 | ALIGN 과 같은 곳에서 잇고 같은 곳에서 못 잇는다 (ALIGN 도 재배선·협상이 없다) |
| 6 제약 | ALIGN 배선기가 읽는 SymmNet, ShieldNet, MultiConnection, DoNotRoute, Route 를 같이 읽는다. 나머지는 ALIGN 도 배선에서 안 본다 |
| 7 계층 | bottom-up — 같은 하위 모듈은 같은 배선이다. GDS 는 ALIGN 파이썬 GDS 와 같은 모양 |
| 8 입력 | 리프 셀 전체 도형이 필요한 것은 같다. 커패시터·가드링은 ALIGN 축소 휠로도 못 했다 |
