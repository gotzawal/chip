...
|---|---|---|
| 전역 배선 | gcell 단위, lp_solve 로 ILP 를 풀어 넷마다 후보 트리를 고른다 | 없다 |
| 상세 배선 | 전역 배선이 정한 영역 안에서 격자 A* | M2~M4 한 격자에서 넷마다 A*, 겹치면 협상(PathFinder) |
| 규칙 | 배선기 안의 PDK 규칙 처리 | 검사기 규칙에 맞춘 트랙별 구간 규칙, 끝에 최소 길이·끝단 간격 정리 |
