...
import { variantGroups, countAssignments, flipPlan, moduleOrder } from "../../../../src/design.mjs";
...
for (const ex of exampleNames()) {
  ...
  console.log(`  배정별 최고 (legalize 후) — 면적 / HPWL(핀 중심) / HPWL 경계 / 점수` +
              `   [legalize 성공 ${best.size}/${nAssign} 배정]`);
  // 60 줄을 다 찍으면 읽을 수 없다. 위아래만 남긴다.
  ...
  for (const [k, v] of ranked) {
    ...
    console.log(`    [${k.padEnd(11)}] ` +
                ...
                `${v.ea.area.toExponential(3)}  ${v.hpC.toFixed(0).padStart(6)}  ${v.hp.toFixed(0).padStart(6)}  ` +
                `${v.sc.toFixed(4)}  (${ranked.findIndex(([q]) => q === k) + 1}위)`);
  }
